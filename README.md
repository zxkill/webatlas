...
```
4) Рекомендация для больших файлов: используйте ночной импорт (COPY → staging) через Celery-задачу `nightly-import-domains-from-zip`.
5) Аудит доменов (модульный аудит со всеми модулями по умолчанию):
```
