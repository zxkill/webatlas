...
- Высокопараллельный аудит доменов (async + threadpool) для быстрого массового сканирования.
...
### Параллельный аудит доменов (скорость и масштаб)

WebAtlas рассчитан на массовый аудит доменов и максимально использует ресурсы ПК/сервера:

- Параллельный запуск аудита доменов через async-окно `audit.concurrency`.
- Параллельная запись результатов в БД через `audit.persist_concurrency`.
- Блокирующие DNS-запросы выполняются в отдельном threadpool, чтобы не тормозить event loop.
- Отдельные настройки `threadpool_workers` и `http_pool_limit` позволяют масштабировать аудит под мощность ПК.

Минимальная конфигурация для ускорения (см. `config.yaml`):

```yaml
audit:
  concurrency: 1000
  persist_concurrency: 200
  threadpool_workers: 128
  http_pool_limit: 1000
  http_pool_limit_per_host: 200
  timeouts:
    total: 2
```

Ключевые слова для поиска: аудит доменов, CMS detection, безопасность сайтов, сканер доменов, инвентаризация веб‑ресурсов.
...
### Индексы БД для ускорения UI

- В PostgreSQL добавлены индексы по доменам, запускам модулей и связям (domain_checks/domain_cms/admin_panels).
- Это ускоряет построение dashboard и отчётов при больших объёмах данных.
