...
WebAtlas — масштабируемая платформа для инвентаризации доменов, определения CMS/фреймворков и будущего поиска
...
- Доступность домена считается подтверждённой только при ответе HTTP 200 по HTTP или HTTPS; при отсутствии 200 остальные
  проверки пропускаются.
- Каждый модуль сохраняет результаты в отдельной таблице (например, availability, Bitrix, TLS) и формирует блок отчёта
  из своих данных.
...
- `timeline` — последние события в удобном формате (для короткого списка “что происходило”)  
  Рекомендуемые поля:
    - `timestamp`
    - `status` (например: `2xx`, `4xx`, `no_response`)
    - `title` (короткая строка для UI)
...
- `entries` — список последних записей (текущий UI использует это для таблицы и счётчика “N шт.”)
...
WebAtlas, аудит доменов, проверка CMS, безопасность веб-сайтов, TLS сертификаты.
